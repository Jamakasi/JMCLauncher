...
import jmc.minecraft.GUIMainFrame;
 ...
 * @author DimanA90
 */
public class BuildMainWindow extends TransparentPanel{
     ...
     * Main method
     */
    public BuildMainWindow(final GUIMainFrame mainframe)
  {
        this.mainframe = mainframe;
        
        BorderLayout groundbl = new BorderLayout();
        setLayout(groundbl);
        ...
        retryButton.addActionListener(new ActionListener() 
        {
            public void actionPerformed(ActionEvent ae) 
            {
                errorLabel.setText("");
                removeAll();
                add(BuildMainWindow.this.buildMainLoginPanel(), "Center");
                validate();
            }
        });
        ...
        offlineButton.addActionListener(new ActionListener() 
        {
            public void actionPerformed(ActionEvent ae) 
            {
                //mainframe.playCached(userName.getText());
            }
        });
        //Launch pressed
...
public void doLogin() {
    setLoggingIn();
    System.out.println("BuildMainWindow.doLogin() start");
    new Thread() {
      public void run() {
        try {
          mainframe.login(userName.getText(), new String(password.getPassword()));
        } catch (Exception e) {
          setError(e.toString());
        }
      }
...
private JPanel buildMainLoginPanel() 
  ...
  private JPanel buildLoginPanel() {
    ...
    titles.add(new TransparentLabel("Выбор сервера:"));
    titles.add(new TransparentLabel("", 4));

   //добавить сохранение и загрузку последнего сервера 
JComboBox comboBoxServer = new JComboBox(GlobalVar.ClientNames);
    //comboBoxServer.setSelectedIndex(0); //загрузка последнего сервера
comboBoxServer.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
                            JComboBox temp = (JComboBox) e.getSource();
                            GlobalVar.CurrentServer = temp.getSelectedIndex();    
                        }
}); 
    values.add(userName);
    values.add(password);
    values.add(comboBoxServer);
...
private TransparentPanel buildOfflinePanel()
  {
    ...
    boolean canPlayOffline = mainframe.canPlayOffline(userName.getText());
...
*/
  private JScrollPane getUpdateNews()
  {
    ...
    try
    {
      ...
      new Thread() {
        public void run() {
          try {
    		  editorPane.setPage(new URL(GlobalVar.NewsURL));
          } catch (Exception e) {
            e.printStackTrace();
  ...
  private TransparentLabel getUpdateLink() {
    ...
    accountLink.addMouseListener(new MouseAdapter() {
      public void mousePressed(MouseEvent arg0) {
        try {
          Utils.openLink(new URL(GlobalVar.DownloadNewLauncherURL).toURI());
        } catch (Exception e) {
          e.printStackTrace();
        }
      }
