...
public class OptionsPanel extends JDialog
{
  ...
  public OptionsPanel(Frame parent)
  {
    ...
    TransparentLabel dirLink = new TransparentLabel(Util.getWorkingDirectory().toString()+ File.separator + Config.workFolderServers[Config.CurrentServer]) {
    ...
    dirLink.addMouseListener(new MouseAdapter() {
      public void mousePressed(MouseEvent arg0) {
        try {
          Util.openLink(new URL("file://" + Util.getWorkingDirectory().getAbsolutePath()+ File.separator + Config.workFolderServers[Config.CurrentServer]).toURI());
        } catch (Exception e) {
          e.printStackTrace();
        }
      }
